...
(defn make-locator
  "Returns a locator from either a String file path, a File object or an InputStream. This locator is threadsafe.
...
(defn lookup
  "Lookup an `ip` given as a String in the given `db` and returns a map with results"
  [^GeoIp2Provider db ip]
  (if-let [city (.country db (InetAddress/getByName ip))]
